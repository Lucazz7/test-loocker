...
class NodeCryptoHash {
  ...
  sha256Hash(message) {
    return _asyncToGenerator(function* () {
      ...
      hash.update(message);
      return (0, _transport.safeBase64)(new Uint8Array(hash.digest()));
    })();
  }
...
function _parseResponse() {
  _parseResponse = _asyncToGenerator(function* (res) {
    var mode = (0, _transport.responseMode)(res.contentType);
    var utf8 = 'utf8';
    var result = yield res.body;

    if (mode === _transport.ResponseMode.string) {
...
class NodeTransport extends _baseTransport.BaseTransport {
  constructor(options) {
    super(options);
  ...
  stream(callback, method, path, queryParams, body, authenticator, options) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var streamPromise = new Promise((resolve, reject) => {
        (0, _transport.trace)("[stream] beginning stream via download url", init);
        ...
        req.on('error', err => {
          if (hasResolved && err.code === 'ECONNRESET') {
            (0, _transport.trace)('ignoring ECONNRESET that occurred after streaming finished', init);
          } else {
            (0, _transport.trace)('streaming error', err);
            reject(err);
          }
        }).on('finish', () => {
          (0, _transport.trace)("[stream] streaming via download url finished", init);
        }).on('socket', socket => {
          (0, _transport.trace)("[stream] setting keepalive on socket", init);
          socket.setKeepAlive(true);
        }).on('abort', () => {
          (0, _transport.trace)("[stream] streaming via download url aborted", init);
        }).on('response', () => {
          (0, _transport.trace)("[stream] got response from download url", init);
        }).on('close', () => {
          (0, _transport.trace)("[stream] request stream closed", init);
        }).pipe(stream).on('error', err => {
          (0, _transport.trace)("[stream] PassThrough stream error", err);
          reject(err);
        }).on('finish', () => {
          (0, _transport.trace)("[stream] PassThrough stream finished", init);
          resolve();
          hasResolved = true;
        }).on('close', () => {
          (0, _transport.trace)("[stream] PassThrough stream closed", init);
        });
      });
  ...
  timeout(options) {
    ...
    return _transport.defaultTimeout;
  }
  ...
  initRequest(method, path, queryParams, body, authenticator, options) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!options.agentTag) {
        options.agentTag = "".concat(_transport.agentPrefix, " ").concat(_constants.lookerVersion);
      }

      var headers = _objectSpread({
        [_transport.LookerAppId]: options.agentTag
      }, options.headers);
