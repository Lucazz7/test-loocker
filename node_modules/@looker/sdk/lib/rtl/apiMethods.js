...
class APIMethods {
  constructor(authSession) {
    var apiVersion = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : _constants.defaultApiVersion;
    this.authSession = authSession;

    _defineProperty(this, "apiPath", '');

    _defineProperty(this, "apiVersion", _constants.defaultApiVersion);

    this.authSession = authSession;
    this.apiVersion = apiVersion;

    if (!('agentTag' in authSession.settings && authSession.settings.agentTag)) {
      authSession.settings.agentTag = "".concat(_transport.agentPrefix, " ").concat(_constants.lookerVersion, ".").concat(this.apiVersion);
    }

    this.apiPath = authSession.settings.base_url === '' ? '' : "".concat(authSession.settings.base_url, "/api/").concat(this.apiVersion);
  }
